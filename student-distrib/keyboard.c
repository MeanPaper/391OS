...
//if alt is pressed, do nothing. alt and keypress will execute the terminal and everything
	if(alt_pressed_cons == 1){
		if(scan_code == F1_pressed && current_term_id != 0){
			...
			terms[1].viewing = 0;
			terms[2].viewing = 0;
			strncpy((int8_t*)terms[current_term_id].terminal_buf, (int8_t*)key_buffer, 128);	// copy the content to the terminal buffer
			terms[current_term_id].key_index = buffer_index;		// storing the current buffer index
